...
function abrirJanelaLeito(leito, iniciais, dx) {
    ...
    // Resgata elementos de abas para aplicar as regras de ocultação/travas
    ...
    // Limpa a zona do checklist para remontar
    ...
    // --- CONSTRUTOR DINÂMICO DO CHECKLIST (PROFILAXIAS, DISPOSITIVOS E BARREIRAS) ---
    // Definição dos itens cruciais baseados no seu modelo clínico
    ...
    if (perfilAtivo === "CHEFIA") {
        ...
        // REGRA DE UX DA ROTINA: Não exibe caixas de preenchimento ruidosas. Exibe o apanhado geral consolidado.
        ...
        itensChecklist.forEach(item => {
            // Simula uma resposta do sistema (marcando alguns itens automáticos como ativos baseado na evolução de ontem)
        ...
        // REGRA DE UX DO INTERNO: Gera os formulários e campos interativos para preenchimento manual à beira do leito
        ...
        // PLANTONISTA também ganha a interface interativa de checagem/ajuste rápido
        let htmlForm = `<div style="display:flex; flex-direction:column; gap:12px;">`;
        itensChecklist.forEach(item => {
            htmlForm += `
                <label style="display:flex; align-items:center; gap:10px; cursor:pointer; background:#f8fafc; padding:8px 12px; border-radius:6px; border:1px solid #e2e8f0;">
